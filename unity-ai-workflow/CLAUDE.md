...
AI-first Unity 2022 LTS game development workflow. This repo is pure infrastructure — rules, agents, skills, workflows, docs, and templates. No Unity project inside. Copy it into your game workspace root and Claude Code reads it as its brain.
...
### Engine & Platform
- **Unity 2022 LTS** (override: `ProjectConfig.yaml → unity_version`)
...
- **UniTask** for async (Cysharp/UniTask package). Use `this.GetCancellationTokenOnDestroy()` for MonoBehaviour-scoped cancellation.
...
- Use `GameDebug` wrapper (stripped from release) — or `Debug.Log()` is acceptable if `GameDebug` is not set up
- Never call Unity API from background threads

### UGS (Unity Gaming Services)
- Backend uses UGS: Authentication, Cloud Save, Economy, Leaderboards, Friends, Lobby, Analytics, Push Notifications
- **Never** hardcode UGS keys or secrets — use Unity Dashboard configuration
- Always wrap UGS calls in `try/catch` — network operations can fail
- Always check `AuthenticationService.Instance.IsSignedIn` before calling other UGS services
...
- Assembly definitions (`.asmdef`) are OPTIONAL but recommended for larger projects
