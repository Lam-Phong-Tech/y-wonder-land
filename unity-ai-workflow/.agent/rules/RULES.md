...
| **Async** | `UniTask` (Cysharp/UniTask package) | `ProjectConfig.yaml → async_pattern` (Coroutines allowed for simple sequences) |
...
- If using async background work, always marshal results back to the main thread via `await UniTask.SwitchToMainThread()` or Unity's `SynchronizationContext`.

### Debugging
- **(OPTIONAL)** Use the project's **`GameDebug`** wrapper class with conditional compilation for better release-build stripping:
```csharp
// ✅ PREFERRED — stripped from release builds (if GameDebug is set up)
GameDebug.Log("Player spawned", GameDebug.Category.Gameplay);

// ✅ ACCEPTABLE — standard Unity logging
Debug.Log("Player spawned");
```
- The `GameDebug` class can optionally be generated during Phase 3 (Project Setup) using the `unity-debugging` skill.
- `Debug.Log()` is acceptable if `GameDebug` is not set up.
...
### Coding Standards
- Follow `docs/CODING_STANDARDS.md` for formatting (Allman braces, 4-space indent, etc.).
...
| **Assembly Definitions** | Assembly definitions (`.asmdef`) are OPTIONAL but recommended for larger projects. They improve compile times and enforce dependency boundaries. |
...
|------|------------|----------------|
| Assembly Definitions | Recommended for larger projects | Optional — scripts can live without `.asmdef` |
...
|------|-----------------|--------|
| **GameDebug or Debug.Log** (use one consistently) | ✅ Always | Structured logging aids debugging |
...
## 9. UGS (Unity Gaming Services) Rules

| Rule | Detail |
|------|--------|
| **No Hardcoded Keys** | Never hardcode UGS keys, secrets, or environment IDs in source code. Use Unity Dashboard configuration. |
| **Error Handling** | Always wrap UGS service calls in `try/catch` — network operations can fail at any time. |
| **Auth Check** | Always check `AuthenticationService.Instance.IsSignedIn` before calling other UGS services. |
| **Initialization** | Initialize UGS with `await UnityServices.InitializeAsync()` before using any service. |
| **Services Used** | Authentication, Cloud Save, Economy, Leaderboards, Friends, Lobby, Analytics, Push Notifications. |
| **Offline Graceful** | Handle offline/disconnected states gracefully — cache locally where possible (e.g. Cloud Save). |
