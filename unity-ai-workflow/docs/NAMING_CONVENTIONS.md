...
| **Material** | M_PascalCase *(recommended)* | `M_PlayerSkin.mat` |
| **Texture** | T_PascalCase_Suffix *(recommended)* | `T_PlayerSkin_Albedo.png` |
| **Normal Map** | T_PascalCase_Normal *(recommended)* | `T_PlayerSkin_Normal.png` |
| **Sprite** | SPR_PascalCase *(recommended)* | `SPR_CoinGold.png` |
| **Animation Clip** | AC_PascalCase *(recommended)* | `AC_PlayerRun.anim` |
| **Animator Controller** | ANIM_PascalCase *(recommended)* | `ANIM_Player.controller` |
| **Audio Clip** | SFX_ or MUS_ *(recommended)* | `SFX_Explosion.wav`, `MUS_MainTheme.ogg` |
| **UXML** | PascalCase | `PlayerHUD.uxml` |
| **USS** | PascalCase | `PlayerHUD.uss` |
| **ScriptableObject** | PascalCase | `SwordData.asset` |
| **Shader** | SH_PascalCase *(recommended)* | `SH_CustomLit.shader` |
...
> **Note:** Asset prefixes (`M_`, `T_`, `SPR_`, `AC_`, `SFX_`, etc.) are **recommended for new files** to improve searchability. Existing files without prefixes do not need to be renamed.

```
Assets/
├── Scripts/Combat/            # C# scripts, organized by domain
├── Scripts/UI/                # UI-related scripts
├── UI/                        # UXML, USS, UI assets
├── Models/                    # 3D models, FBX files
