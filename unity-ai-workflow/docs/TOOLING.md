...
> ⚠️ Unity 6+ only — not available in Unity 2022 LTS. The features below require Unity 6.2+.
...
> ⚠️ Unity 6+ only — not available in Unity 2022 LTS.
...
> ⚠️ Unity 6+ only — not available in Unity 2022 LTS. For Unity 2022 LTS, use the older `com.unity.sentis` package.
...
## UGS Dashboard (Unity Gaming Services)

The [UGS Dashboard](https://cloud.unity.com/) is the web portal for managing backend services used in production. This is available for **all Unity versions** including Unity 2022 LTS.

| Service | Dashboard Section | What You Manage |
|---------|------------------|-----------------|
| **Authentication** | Players → Authentication | Sign-in methods (anonymous, Google, Apple, email), linked accounts, player management |
| **Cloud Save** | LiveOps → Cloud Save | Player data files, access control lists, per-title data quotas |
| **Economy** | LiveOps → Economy | Virtual currencies, inventory items, store listings, real-money purchase configs |
| **Leaderboards** | LiveOps → Leaderboards | Leaderboard definitions, score configs, reset schedules, bucketing |
| **Friends** | Social → Friends | Relationship configs, presence settings, block lists |
| **Lobby** | Multiplayer → Lobby | Lobby configs, query filters, max player limits |
| **Analytics** | Analytics → Events | Custom event schemas, funnels, dashboards, data export |
| **Push Notifications** | LiveOps → Push Notifications | Notification campaigns, segments, scheduling |

### Workflow Integration
- **Pre-production**: Set up UGS project in the Dashboard, enable required services.
- **Production**: Network Engineer configures auth flows + data schemas; reference `docs/API_CONTRACTS.md` and `docs/DATA_SCHEMA.md`.
- **Testing**: Use Dashboard to inspect player data, reset leaderboards, test economy balances.
- **Analytics**: Define custom events in Dashboard → instrument in code via `AnalyticsService.Instance.RecordEvent()`.

> **Async pattern**: All UGS SDK calls are async. Use `UniTask` for zero-allocation awaiting:
> ```csharp
