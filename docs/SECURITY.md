...
Dù extension web debit đã pass full isolated validation, cho tới khi candidate được deploy riêng, migration/link account được duyệt và có QA riêng, không chuyển `open` hoặc chạy giao dịch ví trên identity thật hiện tại. Production chưa bật `WEB_POINT_WALLET_DEBIT_ENABLED`; bước production tiếp theo phải được duyệt riêng và giữ debit tắt. Việc saga nội bộ chạy đúng không đồng nghĩa rút USDT bên ngoài, YWH hay hoa hồng đã hoàn chỉnh.
