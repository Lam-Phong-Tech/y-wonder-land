...
Nền authority v3 và extension web debit `Point -> USDT` đã pass full validator cô lập trên source + SQLite production bản sao: unique `User.id -> gamePlayerId`, linked-wallet Point freeze, signed balance identity check, callback v2, immutable Admin rate version, `reserve/capture/release`, Prisma migration/DB E2E, Next build và fault E2E. Nền này đã deploy production ở trạng thái dormant: schema cộng thêm đã áp nhưng các bảng link/conversion/debit/reservation đều rỗng, và cả game/web đặt `WEB_POINT_WALLET_DEBIT_ENABLED=false`. Playbook synthetic fixed-rate `0,06 USDT -> 1 Point` tại `docs/QA/WEB_POINT_NO_MONEY_CANARY.md` vẫn bị giữ lại; test mới phải dùng version tỷ giá Admin đang active.
...
Dù extension web debit đã pass full isolated validation và nền candidate đã deploy dormant, cho tới khi migration/link account được duyệt và có QA riêng, không chuyển `open` hoặc chạy giao dịch ví trên identity thật hiện tại. Production vẫn đặt `WEB_POINT_WALLET_DEBIT_ENABLED=false`; bước tiếp theo là canary không tiền được duyệt riêng, không phải bật tiền thật. Việc saga nội bộ chạy đúng không đồng nghĩa rút USDT bên ngoài, YWH hay hoa hồng đã hoàn chỉnh.
