...
- Web debit candidate giữ một browser request ID bền, journal deterministic và chỉ cho một operation chưa terminal trên cả hai chiều. Trigger SQLite chéo là lớp chống race cuối cùng khi nhiều tab cùng gửi.
- Settlement USDT `PENDING` phải tồn tại trước capture nhưng không được cộng vào `balanceUsdt` spendable. Chỉ transaction SQLite sau response `CAPTURED` mới cộng USDT và đánh dấu thành công; settlement không tạo được thì release cùng reservation. Journal đã tồn tại thì cấm release để tránh vừa hoàn Point vừa giữ USDT.
- Fee debit không được kế thừa hằng số cũ. Candidate fail closed nếu thiếu `WEB_POINT_DEBIT_FEE_BPS`; `Point -> YWH` tiếp tục tắt.
...
Nền authority v3 và extension web debit `Point -> USDT` đã pass full validator cô lập trên source + SQLite production bản sao: unique `User.id -> gamePlayerId`, linked-wallet Point freeze, signed balance identity check, callback v2, immutable Admin rate version, `reserve/capture/release`, Prisma migration/DB E2E, Next build và fault E2E. Nền này đã deploy production; cả game/web vẫn đặt `WEB_POINT_WALLET_DEBIT_ENABLED=false`.

Ngày 17/07/2026, canary kỹ thuật không tiền mới đã chạy trên profile QA riêng `WalletQA2026`, không dùng lại account thật. Executor ghim mapping một-một, signed balance, tỷ giá Admin active `26,5 Point/USDT` và exact quote; journal funding dùng loại riêng `QA_SYNTHETIC_USDT_FUNDING`, không dùng `USDT_DEPOSIT`. Funding `2 USDT` synthetic rồi thao tác đổi thật trên UI web tạo đúng một SWAP/conversion/outbox và một game ledger `+53`, đưa Point `5000 -> 5053`, remainder `0`. Replay cùng callback trả `duplicate=true`; HMAC sai/quá hạn, ngoài allowlist, payload conflict, sai player và zero amount đều bị từ chối không mutation. Real deposit count và deposited/withdrawn totals vẫn `0`.
...
Canary QA mới chỉ cho phép kết luận `technical no-money canary complete`. Không chuyển `open` hoặc tự gọi đây là giao dịch tiền thật: production vẫn exact-one QA, loopback-only, public callback `404` và debit `false`. Trước khi mở rộng phải chạy canary nhà cung cấp thật cực nhỏ khi được duyệt, hoàn thiện migration account legacy, chuyển positive gameplay grants sang action/claim server-authoritative, có cảnh báo/đối soát/reversal vận hành và chốt YWH/hoa hồng. Việc saga nội bộ chạy đúng không đồng nghĩa rút USDT bên ngoài, YWH hay hoa hồng đã hoàn chỉnh.
...
- [x] Exporter fail-closed đã chạy production read-only bằng double snapshot; remediation/reconciliation hậu xử lý đạt `BLOCKED=0`, không làm drift hoặc mutation ngoài operation được duyệt.
- [x/~] PostgreSQL game là ledger authoritative; worksheet successor đã phân loại đủ account còn hiệu lực nhưng policy tiếp tục `DEFER_ACCOUNT_LINK`, nên chưa migrate số dư legacy.
- [x/~] Rate version, integer micros, rounding journal và retry đã chạy production ở canary QA với rate `26,5`, quote `53` nguyên và remainder `0`; semantics YWH/rút vẫn chờ chốt.
...
- [x] Canary QA đã chứng minh thao tác UI web tạo đúng một SWAP `SUCCESS`, conversion, outbox `SENT` và game ledger cùng source transaction; funding đầu vào là synthetic, không phải tiền thật.
...
- [x] Cô lập grant scope đã chuyển sang đúng profile QA riêng `WalletQA2026`, mapping một-một và exact allowlist; không tiếp tục dùng account thật `Nhien345` làm QA.
...
- [x] Client QA mới giữ đúng PostgreSQL authoritative `5053` qua EXE logout/login, EXE -> APK, APK full close/relogin và APK -> EXE; phiên cũ bị đẩy và có toast ở cả hai chiều.
...
- [x] Web production đang ở Next.js `15.5.20`, build `2rdR_xG8o4G1uonGEYEg0`; route guards, SSO, authenticated wallet, cron, rollback và postflight đều pass. Top-up vẫn canary đúng một QA, chưa dùng tiền thật.
- [x/~] Funding synthetic có kiểm soát, thao tác đổi thật trên UI web, retry cùng transaction, realtime, relogin và đổi EXE/APK đã đạt. Restart/response-loss đã pass trên production artifact cô lập; giao dịch nhà cung cấp thật cực nhỏ chưa thực hiện.
- [x] Report production đã đối soát funding/SWAP/conversion/outbox với đúng một `game_transactions` ledger; duplicate replay và fault matrix không mutation đều pass.
