...
- **Cập nhật 10/07/2026 - inventory/economy gameplay + vị trí farm `[x]`:** test của anh xác nhận cây/bù thời gian và Point shop lưu đúng, đồng thời từng phát hiện hạt đã gieo quay lại túi và nước đã múc biến mất sau relog. Nguyên nhân là gameplay `AddItem/RemoveItem` chỉ sửa local rồi bootstrap nạp inventory server cũ. `GameplayMutationSync` nay gửi tuần tự mọi delta inventory/economy có idempotency; bootstrap/shop/logout chờ flush. Phạm vi gồm hạt, nước, thu hoạch, gỗ/đá/cá, thức ăn, thú, phân bón, quà, vật liệu xây và Point/UPoint gameplay. Logout lưu pose farm trước khi hủy nhân vật và profile cũ ưu tiên pose đó. C# compile, full Phase 1 smoke, API relog test 20 nước/1 hạt sầu riêng/5030 Point và bản Unity runtime đều pass; ngày 10/07 anh xác nhận vận hành ổn. Cache A -> B -> A vẫn chờ test riêng.
- **Cập nhật 10/07/2026 - cache theo `playerId` đã code, chờ test:** thêm `PlayerScopedPrefs` để key gameplay tách theo account; legacy save chỉ một account được nhận, online-only chưa login không đọc/ghi save chung. Khi identity đổi, auth phát event lưu scope cũ rồi nạp scope mới cho Point/túi/tool/EXP, vị trí, farm/cây, ô lát/công trình, thú, lượt câu/đào, điểm danh/vòng quay. C# compile pass, còn warning auto-sprint cũ. **Chưa tick `[x]`:** cần test A -> B -> A trên cùng PC, xác nhận dữ liệu/world đổi đúng và A lấy lại trạng thái cũ.
- **Cập nhật 10/07/2026 - shop `[x]`, click xuyên biển `[x]`, chốt VPS game:** anh xác nhận `42.96.18.14` là VPS riêng cho game, được phép cài Node game-server + PostgreSQL và không có dịch vụ cần giữ; sau này `api.ywonder.net` sẽ trỏ về đây, còn web account ở máy web riêng. TCP `22` và bắt tay SSH đã hoạt động; banner `OpenSSH_8.9p1 Ubuntu-3ubuntu0.15`, hỗ trợ password/public key. Chưa đăng nhập nên OS/tài nguyên thật vẫn phải audit, chưa deploy/đổi URL Unity. Credential giữ ngoài repo. Kế hoạch nghiệm thu/rollback ở `docs/VPS_GAME_DEPLOYMENT_PLAN.md`.
...
- **Cập nhật 10/07/2026 - realtime action sync + audit dữ liệu Phase 1:** checkpoint account/session/public tunnel đã commit ở `0c545db`; commit `4bde50d` gửi đúng state, tốc độ và dụng cụ cho jump/swim/hoe/mining/chop/water/fish/feed/plant. Smoke backend, Unity compile và test runtime hai client của anh đều đã pass. Audit tại `docs/PHASE1_STATE_SYNC_AUDIT.md` chốt dự án vẫn ở **cuối Giai đoạn 1**, chưa sang PostgreSQL; resource `30c6fc9` và shop mới nhất đã server-authoritative, còn cache theo playerId, farm/câu cá/daily limits và test 5-20 người.
