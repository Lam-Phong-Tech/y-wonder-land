...
- **Cập nhật 10/07/2026 - shop `[x]`, click xuyên biển `[x]`, chốt VPS game:** anh xác nhận `42.96.18.14` là VPS riêng cho game, được phép cài Node game-server + PostgreSQL, dự kiến Ubuntu Server 24.04 LTS và không có dịch vụ cần giữ; sau này `api.ywonder.net` sẽ trỏ về đây, còn web account ở máy web riêng. OS/tài nguyên thật vẫn phải audit qua SSH. Lần kiểm tra mới từ máy làm việc chưa tới được `22/80/443/3000`, DNS vẫn về `45.119.83.233`, nên chưa login/deploy/đổi URL Unity. Credential giữ ngoài repo. Kế hoạch có cổng nghiệm thu và rollback nằm tại `docs/VPS_GAME_DEPLOYMENT_PLAN.md`; trong lúc chờ SSH, bước code tiếp theo vẫn là cache theo `playerId`.
