...
- **Cập nhật 11/07/2026 - SSH deploy VPS `[x]`:** đã tạo user không đặc quyền `deploy`, gắn ED25519 public key và xác minh đăng nhập batch thành công. `/home/deploy/.ssh` mode `700`, `authorized_keys` mode `600`, đều thuộc `deploy:deploy`. Chưa khóa `root/password`, chưa đổi sshd/UFW và chưa cài package nên vẫn có rollback. Password/private key không nằm trong repo. Bước source kế tiếp là hoàn thiện PostgreSQL adapter + migration account trước khi deploy staging.
