...
- **Cập nhật 07/07/2026 - bỏ tâm tương tác, tap trực tiếp vật thể:** theo yêu cầu mới từ khách, flow tương tác thế giới đã chuyển từ quét theo tâm màn hình sang tap/click trực tiếp lên vật thể trong tầm gần. `FarmInteractionController` mặc định dùng `useDirectTapInteraction = true`, giới hạn `directTapMaxRange = 2m`, chỉ hiện UI action khi người chơi bấm đúng cây/đá/hồ nước/điểm câu cá/shop/chuồng/ô build đủ gần; tap xa hoặc tap trống không mở prompt. `GameHUD.uss` đã ẩn `Crosshair`. **Cần test:** APK/EXE, đứng trong khoảng 1-2m và tap trực tiếp vật thể để UI hiện; bấm action vẫn thao tác như cũ; đi xa/tap trống không hiện prompt.
