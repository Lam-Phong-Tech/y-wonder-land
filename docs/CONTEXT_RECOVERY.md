...
- **Cập nhật 10/07/2026 - shop `[x]` + chặn click nền xuyên biển `[x]`:** anh đã test EXE/APK/relogin và xác nhận shop nguyên tử hoạt động khá tốt, nên task shop/Loop E được chốt `[x]`; reconnect đôi lúc hơi lâu nhưng hiện không chặn demo. `FarmInteractionController` cũng giảm khoảng direct-tap được đi tiếp sau collider đặc từ gần `1.7m` xuống sai số bề mặt `0.05m`; anh đã test runtime và xác nhận nền Thành phố chặn WaterSource/FishingSpot dưới đảo trong khi click nước thật vẫn hoạt động. VPS ứng viên `42.96.18.14` chưa đăng nhập/thay đổi: cổng `22` từng phản hồi ở lần kiểm tra đầu nhưng lần kiểm tra mới từ máy làm việc không tới được `22/80/443/3000`; DNS `api.ywonder.net` vẫn về `45.119.83.233`. Credential giữ ngoài repo; cần owner xác nhận IP/firewall trước khi audit. **Bước tiếp theo:** tách cache theo `playerId`.
