...
- **Cập nhật 10/07/2026 - realtime action sync + audit dữ liệu Phase 1:** checkpoint account/session/public tunnel đã commit ở `0c545db`; đồng bộ realtime nay gửi đúng state, tốc độ và dụng cụ cho jump/swim/hoe/mining/chop/water/fish/feed/plant, commit `4bde50d`. Smoke test backend tạm đã pass `Jump` và `Mining + Pickaxe`; Unity import không có lỗi C#, còn chờ build 2 client xác nhận runtime. Audit mới tại `docs/PHASE1_STATE_SYNC_AUDIT.md` chốt dự án đang ở **cuối Giai đoạn 1**, chưa sang PostgreSQL: profile đã đọc/ghi server, economy/inventory mới load từ bootstrap, còn shop/farm/daily limits vẫn ghi PlayerPrefs và chưa server-authoritative. **Bước tiếp theo:** API shop buy/sell nguyên tử + idempotency, sau đó tách cache theo playerId, nối farm/daily limits và test 20 người.
