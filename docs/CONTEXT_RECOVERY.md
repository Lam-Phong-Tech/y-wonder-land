...
- **Cập nhật 11/07/2026 - private staging VPS `[x]`:** đã cài Node `24.18.0` LTS từ binary chính thức có kiểm SHA-256 và Caddy stable `2.11.4`; deploy backend commit `ebc9982`. `ywonder-game-server.service` và Caddy đều `enabled/active`; health trả store `postgres`; full Phase 1 register/login/bootstrap/shop/persistence/idempotency/realtime smoke qua Caddy pass. Node bind `127.0.0.1:3000`; sau khi phát hiện Caddy vẫn tạo socket wildcard dù site address dùng loopback, đã thêm `bind 127.0.0.1` và xác minh Caddy chỉ nghe `127.0.0.1:8080`. Từ máy ngoài chỉ SSH `22` mở; `80/443/3000/5432/8080` đều đóng. Chưa import JSON cũ, chưa đổi DNS/Unity URL. Bước tiếp: hardening API, xác nhận DNS/dependency web, public HTTPS/WSS và reboot/load acceptance.
