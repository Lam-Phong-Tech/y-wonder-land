...
> Update 2026-07-10: atomic shop buy/sell and the City direct-tap fix are both runtime accepted. VPS `42.96.18.14` is now confirmed by anh as the dedicated game server, allowed to host Node + PostgreSQL, expected to be Ubuntu Server 24.04 LTS with no services to preserve; `api.ywonder.net` will later move to it while the web account host stays separate. Actual OS/resources still require a read-only SSH audit. Latest work-machine checks cannot reach `22/80/443/3000`, and DNS still resolves to `45.119.83.233`, so no login/deploy/client URL change has occurred. Credentials stay outside the repo. Follow `docs/VPS_GAME_DEPLOYMENT_PLAN.md`; meanwhile the next game-data slice remains player-scoped cache.
