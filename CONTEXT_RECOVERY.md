...
> Update 2026-07-10: runtime found that purchased seeds consumed by planting returned after relog and scooped water disappeared, because shop inventory was server-owned but normal `InventoryManager.AddItem/RemoveItem` stayed local. Added `GameplayMutationSync`: all authenticated inventory and economy deltas are serialized to `inventory/adjust` / `economy/apply` with idempotency, and bootstrap/shop/logout flush them before applying a server snapshot. This covers seeds, water, crops, wood/stone/fish, food, animals, fertilizer, gifts, build materials and gameplay Point/UPoint outside shop. Logout now saves the farm pose before destroying the player; existing profiles prefer that pose. C# compile, full Phase 1 smoke and a dedicated water/seed/Point relog API test pass. Anh tested the new Unity flow on 10/07 and confirmed it works correctly, so gameplay delta sync and farm-position restore are now `[x]`. Player-scope A -> B -> A isolation remains `[~]` until explicitly tested.

> Update 2026-07-10: player-scoped local cache code is implemented and compiles, but remains `[~]` until runtime acceptance. `PlayerScopedPrefs` isolates gameplay keys by authenticated `playerId`, grants each legacy key to at most one account, and blocks shared gameplay reads/writes before login in online-only mode. Auth transitions save the old scope and reload the new scope across economy/inventory/tools/EXP, position, farm/crops, placed tiles/buildings, animals, fishing/mining, attendance and wheel state. Test A -> B -> A on one PC before marking complete. VPS SSH is now reachable, but authenticated audit/deploy has not started.

> Update 2026-07-10: atomic shop buy/sell and the City direct-tap fix are both runtime accepted. VPS `42.96.18.14` is confirmed as the dedicated game server, allowed to host Node + PostgreSQL; `api.ywonder.net` will later move to it while the web account host stays separate. TCP `22` and the SSH handshake now work from the development machine; banner is `OpenSSH_8.9p1 Ubuntu-3ubuntu0.15` and password/public-key auth is offered. Actual OS/resources still require authenticated read-only audit; no deploy/client URL change has occurred. Credentials stay outside the repo. Follow `docs/VPS_GAME_DEPLOYMENT_PLAN.md`.
...
> Update 2026-07-10: Phase 1 is at its final demo-online stage, not PostgreSQL staging yet. Commit `0c545db` checkpoints local accounts, public tunnel smoke coverage, and single-session replacement. Commit `4bde50d` relays active animation state, speed, and held tool; Node smoke, Unity compile, and the later two-client runtime test all passed. `docs/PHASE1_STATE_SYNC_AUDIT.md` tracks remaining ownership gaps. The newer shop/resource checkpoints are recorded above; the next backend slice is player-scoped cache, then remaining farm/daily-limit sync and 5-20 player acceptance testing.
