...
> Update 2026-07-10: login error classification and shared resource ownership are now implemented. Commit `a40882f` preserves backend JSON error codes so an unknown account or wrong password no longer appears as a server outage. The current uncommitted resource slice makes Node authoritative for tree/rock claims in `city/mine`: one winner, atomic inventory + mining-limit write, depletion broadcast, late-join snapshot, and 20-second respawn. Temporary backend tests passed both `phase1SmokeTest.js` and the expanded multi-client `realtimeSmokeTest.js`; the same REST/WebSocket/resource smoke passed through the refreshed public Quick Tunnel, and Unity compiled `Assembly-CSharp` successfully. Runtime acceptance still required: build two clients, mine the same rock, verify only one reward and synchronized disappearance/respawn. Shared resource state is RAM-only and resets when Node restarts; PostgreSQL/production persistence remains Phase 2.

> Update 2026-07-10: Phase 1 is at its final demo-online stage, not PostgreSQL staging yet. Commit `0c545db` checkpoints local accounts, public tunnel smoke coverage, and single-session replacement. Commit `4bde50d` relays the active animation state, speed, and held tool so remote players can show jump, swim, farming/mining/chopping and related actions; Node smoke passed Jump and Mining + Pickaxe, and Unity imported the scripts without C# errors. `docs/PHASE1_STATE_SYNC_AUDIT.md` records the remaining blocker: profile is server read/write, but economy/inventory are bootstrap-read only and shop/farm/daily limits still mutate PlayerPrefs. Next vertical slice is an atomic server-authoritative shop transaction, followed by player-scoped cache, farm/daily-limit sync, and 20-player acceptance testing.
