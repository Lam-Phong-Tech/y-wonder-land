...
> Update 2026-07-10: atomic shop buy/sell is now `[x]`. The generated 109-item/8-shop catalog, `POST /player/shop/transaction`, idempotent retry and server-only economy/inventory application passed temp/public smoke plus anh's EXE/APK/relogin test; he reports the flow works well. Reconnect is occasionally slow but currently non-blocking. The follow-up City direct-tap fix also passed runtime: surface passthrough was reduced from roughly `1.7m` to `0.05m`, so ground blocks water/fishing colliders below the island while actual water targets remain usable. Candidate VPS `42.96.18.14` remains untouched: SSH `22` responded on an earlier check, but the latest check from the work machine cannot reach `22/80/443/3000`, and `api.ywonder.net` still resolves to `45.119.83.233`. Credentials stay outside the repo; owner/infra must confirm the IP and firewall before access. Next backend slice is player-scoped cache.
