...
> Update 2026-07-07: direct tap world interaction. Customer requested no more center-target/crosshair for object interaction. `FarmInteractionController` now defaults to direct tap/click targeting with `useDirectTapInteraction = true` and `directTapMaxRange = 3.5m`: action UI appears only when the player taps a nearby tree, rock, water/fishing spot, shop, pen, or farm/build cell. Empty/far taps should not open prompts, and the action UI now auto-hides when the player walks out of range from the selected target. `GameHUD.uss` hides the HUD crosshair. Needs APK/EXE test within roughly 3.5m, then tap action UI and confirm old behavior still executes.
