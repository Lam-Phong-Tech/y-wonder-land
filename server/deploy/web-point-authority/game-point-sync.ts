...
async function markFailure(id: string, attempts: number, error: string, permanent: boolean) {
  ...
  await db.$transaction(async (tx) => {
    const changed = await tx.gamePointSyncOutbox.updateMany({
    ...
    await tx.gamePointConversion.updateMany({
      where: { outboxId: id, status: { notIn: ["SENT", "REFUNDED"] } },
      data: { status, lastError: normalizedError },
    });
  });
}

function validateCreditResponse(body: any, input: {
  transactionId: string;
  webUserId: string;
  expectedPlayerId: string;
  pointAmount: string;
  ...
  if (body?.ok !== true
      ...
      || !Number.isSafeInteger(body?.economy?.pos) || body.economy.pos < 0
      || transaction?.ref !== input.transactionId
      || transaction?.webUserId !== input.webUserId
      || transaction?.expectedPlayerId !== input.expectedPlayerId
      || transaction?.pointAmount !== input.pointAmount
      || transaction?.source !== input.source) {
    throw new Error("INVALID_GAME_POINT_SYNC_RESPONSE");
  }
  return body as { duplicate?: boolean };
}

async function markSent(id: string, sourceTransactionId: string, attempts: number) {
  const now = new Date();
  await db.$transaction(async (tx) => {
    const [outbox, conversion, transaction] = await Promise.all([
      tx.gamePointSyncOutbox.findUnique({ where: { id }, select: { status: true, attempts: true } }),
      tx.gamePointConversion.findUnique({ where: { outboxId: id }, select: { status: true } }),
      tx.transaction.findUnique({ where: { id: sourceTransactionId }, select: { status: true } }),
    ]);
    if (!outbox || !conversion || !transaction) throw new Error("GAME_POINT_SETTLEMENT_JOURNAL_MISSING");
    if (outbox.status === "SENT" && conversion.status === "SENT" && transaction.status === "SUCCESS") return;
    if (conversion.status === "REFUNDED") throw new Error("GAME_POINT_CONVERSION_ALREADY_REFUNDED");
    if (!["PENDING", "SUCCESS"].includes(transaction.status)) {
      throw new Error("INVALID_GAME_POINT_TRANSACTION_STATE");
    }

    await tx.gamePointSyncOutbox.update({
      where: { id },
      data: {
        status: "SENT",
        attempts: Math.max(outbox.attempts, attempts, 1),
        lastError: null,
        sentAt: now,
        nextAttemptAt: now,
      },
    });
    await tx.gamePointConversion.update({
      where: { outboxId: id },
      data: { status: "SENT", lastError: null, sentAt: now },
    });
...
export async function dispatchGamePointSyncOutboxById(id: string): Promise<DispatchResult> {
  ...
  if (record.status === "FAILED") {
    return { ok: false, permanent: true, error: record.lastError || "FAILED" };
  }
  if (!topupEnabled()) {
    return { ok: false, permanent: false, error: "GAME_POINT_SYNC_DISABLED" };
  }
  ...
  const linked = await db.gamePointLinkedAccount.findUnique({
    where: { userId: record.user.id },
    select: { userId: true, gamePlayerId: true },
  });
  const attempts = record.attempts + 1;
  if (!linked) {
    await markFailure(record.id, attempts, "GAME_POINT_LINK_REQUIRED", false);
    return { ok: false, permanent: false, error: "GAME_POINT_LINK_REQUIRED" };
  }
  ...
  try {
    ...
    const input = {
      ...
      expectedPlayerId: linked.gamePlayerId,
    ...
    if (response.ok) {
      const body = validateCreditResponse(await response.json().catch(() => null), input);
      await markSent(record.id, record.sourceTransactionId, attempts);
...
export async function retryPendingGamePointSync(limit = 25) {
  ...
  if (!topupEnabled() || (!openMode && canaryIds.length === 0)) {
...
export async function getGamePointSyncHealth() {
  const [outboxPending, outboxFailed, conversionUnresolved, oldest] = await Promise.all([
    db.gamePointSyncOutbox.count({ where: { status: { in: ["PENDING", "RETRY"] } } }),
    db.gamePointSyncOutbox.count({ where: { status: "FAILED" } }),
    db.gamePointConversion.count({ where: { status: { notIn: ["SENT", "REFUNDED"] } } }),
    db.gamePointConversion.findFirst({
      where: { status: { notIn: ["SENT", "REFUNDED"] } },
      orderBy: { createdAt: "asc" },
