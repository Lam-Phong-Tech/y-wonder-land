...
- `[~]` Hạ tầng game chính thức sếp cấp ngày 10/07: VPS riêng `42.96.18.14`, tài khoản bàn giao ban đầu `root`, mật khẩu giữ riêng ngoài repo. Anh xác nhận được phép cài game-server + PostgreSQL và hiện không có dịch vụ cần giữ; sau này chỉ `api.ywonder.net` sẽ trỏ về IP này, còn website/web account giữ riêng. TCP `22` và bắt tay SSH đã xác nhận hoạt động; banner là `OpenSSH_8.9p1 Ubuntu-3ubuntu0.15`, hỗ trợ `publickey,password`. Chưa đăng nhập nên OS/tài nguyên thực tế vẫn chưa audit, chưa deploy và chưa đổi URL build. Kế hoạch đầy đủ ở `docs/VPS_GAME_DEPLOYMENT_PLAN.md`: audit read-only -> harden/cài dịch vụ -> PostgreSQL -> deploy staging -> DNS/HTTPS/WSS -> đổi Unity + rollback.
