...
- `[~]` Hạ tầng game chính thức sếp cấp ngày 10/07: VPS riêng `42.96.18.14`, tài khoản bàn giao ban đầu `root`, mật khẩu giữ riêng ngoài repo. Anh xác nhận được phép cài game-server + PostgreSQL, máy dự kiến Ubuntu Server 24.04 LTS và hiện không có dịch vụ cần giữ; sau này chỉ `api.ywonder.net` sẽ trỏ về IP này, còn website/web account giữ riêng. OS thực tế vẫn phải audit qua SSH. Lần kiểm tra mới từ máy làm việc chưa tới được `22/80/443/3000`, DNS `api.ywonder.net` vẫn về `45.119.83.233`, nên chưa đăng nhập/chưa đổi build. Kế hoạch đầy đủ ở `docs/VPS_GAME_DEPLOYMENT_PLAN.md`: mở SSH -> audit read-only -> hoàn thiện PostgreSQL local -> harden/cài dịch vụ -> deploy staging -> DNS/HTTPS/WSS -> đổi Unity + rollback.
