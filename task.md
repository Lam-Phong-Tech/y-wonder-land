...
- `[~]` Đồng bộ tài nguyên cây/đá ở room công cộng `city/mine`: server giữ trạng thái theo `resourceId`, chỉ người claim đầu tiên được cộng thưởng, ghi inventory + lượt đào nguyên tử/idempotent, broadcast biến mất, snapshot cho người vào sau và hồi sinh sau 20 giây. Unity chỉ cộng túi đồ sau khi server xác nhận; mất kết nối không tự cộng local. Smoke test nhiều client đã pass cả backend temp lẫn Quick Tunnel public, Unity compile sạch; chờ anh build EXE/APK xác nhận hai máy cùng thấy đá/cây biến mất và hồi lại rồi mới tick `[x]`.
- `[x]` Đã audit tài nguyên/tiền/túi đồ/shop/farm-state tại `docs/PHASE1_STATE_SYNC_AUDIT.md`: profile có đọc/ghi server; economy/inventory phần lớn mới load từ bootstrap; riêng khai thác cây/đá public đã có lát server-authoritative đầu tiên. Shop, farm/crop/animal và giới hạn câu cá vẫn local; PostgreSQL adapter còn là scaffold. Kết luận dự án ở cuối Giai đoạn 1 nhưng chưa hoàn tất toàn bộ vòng dữ liệu server-authoritative.
- `[ ]` P1 tiếp theo: làm một API shop buy/sell nguyên tử có idempotency, server kiểm giá/item và trả economy + inventory sau giao dịch; Unity không tự trừ/cộng local khi online.
- `[ ]` Tách các cache gameplay theo `playerId` để đổi account trên cùng máy không dùng chung Point/inventory/farm/build/animal/daily-limit local.
