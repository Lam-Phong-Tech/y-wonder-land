...
>
> Trạng thái 14/07/2026: PostgreSQL production, hardening/reboot, Nginx `/game-api`, HTTPS/WSS, full Phase 1 và bài tải tự động 20 client từ mạng ngoài đều đã pass. Browser SSO public và đúng artifact EXE/APK đã pass đăng nhập account cũ, đổi account, đăng ký mới + OTP/mã giới thiệu, tạo nhân vật một lần, relogin/cross-device và thay phiên `4008`; local/web auth vẫn song song để rollback. P0 phiên đơn + chống ghi đè farm đã deploy production bằng release `21cc20d2a827e5327429cf5f0ecf67a6b67fdf79`; post-deploy EXE/APK đã pass A -> B -> A, xây dựng, ruộng/cây/nước, túi đồ, chuồng/thú và gửi bù sau đóng app đột ngột. Cổng hiện tại là retest hai hotfix gameplay mới (hủy cho thú ăn và chết/héo trực tiếp ở 0%) rồi đóng RC cho tester.
...
- `[~]` **Hotfix gameplay phát hiện sau nghiệm thu Browser SSO - chờ build test:** đã khóa `FarmInteractionController` về đúng controller `FarmManager` sở hữu timed coroutine, nên nút hủy cho thú ăn phải dừng cả animation/coroutine, mở khóa nhân vật và hoàn thức ăn thay vì vẫn cho ăn. Cây/thú nay chết hoặc héo ngay khi thanh nước/đói về `0%` cả lúc tutorial đang chạy; trạng thái biến mất được lưu ngay vào snapshot farm để không sống lại khi đổi thiết bị. `Assembly-CSharp` compile pass, chỉ còn warning auto-sprint cũ; cần test Editor/EXE/APK trước khi đổi `[x]`.
