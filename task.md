...
0. `[~]` **Ví Point chung:** production dry-run, worksheet và policy approval validator đã đạt; 21/21 decision được duyệt nhưng không có DB mutation. Việc tiếp theo là chuẩn bị change riêng cho reversal synthetic `3 Point` có audit và báo cáo residual `ROUND_HALF_EVEN`, không tự chạy; account link và balance migration tiếp tục defer. Sau mỗi change được duyệt phải chạy lại dry-run tới `BLOCKED=0`; song song mới hoàn thiện web-side reserve/capture/release outbox cho `Point -> USDT/YWH` trong môi trường cô lập. Migration/deploy luôn là lượt riêng có backup/rollback.
