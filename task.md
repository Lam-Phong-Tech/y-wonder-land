...
>
> Deploy dormant authority v3 đã đạt `[x/~]`: game release `a22312df3aee5701a31aa502d2fea3728546b2b1`, web release `/var/www/ywonder-releases/point-v3-a22312df`, build `2rdR_xG8o4G1uonGEYEg0`. Migration PostgreSQL `006` và schema SQLite authority/debit đã áp cộng thêm, toàn bộ bảng link/conversion/debit/reservation mới vẫn `0` row. Cả hai service đặt `WEB_POINT_WALLET_DEBIT_ENABLED=false`; public credit/reserve `404`. Backup root-only `/root/ywonder-point-backups/point-wallet-dormant-20260717T050203Z-a22312df`, manifest hậu switch SHA-256 `473bffad9ff2199d27fcb50f02c88f44d8f2a4aafc6088e3481f33427b3ca88f`. Hai rollback tự động do lỗi smoke harness đã phục hồi đúng release/env/health và không đổi balance; lần switch cuối pass regression web/SSO/cron/dormant route, ổn định 45 giây restart `0`, temp `0`. Không payment/conversion/debit/link/migrate/open/YWH. Cổng tiếp theo là identity QA riêng và canary không tiền được duyệt riêng.
>
...
- `[x/~]` Reconciliation read-only đã chốt `BLOCKED=0`; successor policy checksum-pinned đã duyệt đủ 17 decision còn hiệu lực nhưng không cấp quyền ghi/link/migrate. Web saga `Point -> USDT` đã pass full isolated validator và nền candidate đã deploy dormant với debit tắt; chưa kích hoạt luồng reserve/journal/capture/release cho người dùng. `Point -> YWH`, migration/link từng account, Point transfer và outbox hoa hồng YWH vẫn chờ contract/phê duyệt.
...
0. `[~]` **Ví Point chung:** remediation, client acceptance, fresh reconciliation, successor policy, full isolated validator và deploy nền dormant đều đạt; migration vẫn không được cấp quyền. Việc tiếp theo là tạo identity QA riêng, chụp baseline mapping/số dư rồi xin duyệt canary không tiền qua đúng journal/outbox/reservation trên release active. `Point -> YWH`, transfer, rút USDT bên ngoài và hoa hồng vẫn chưa hoàn tất. Giữ `WEB_POINT_WALLET_DEBIT_ENABLED=false`, `mode=canary`, public callback `404`, không link/migrate account legacy, không mở `open` và không thử tiền thật.
...
6. `[~]` **VÍ CHUNG - authority v3 đã deploy dormant:** Point web/game là một ví; game PostgreSQL đã được chọn làm ledger. `USDT -> Point` dùng rate version Admin; reserve/capture/release game đã pass JSON/PostgreSQL race/restart; web saga `Point -> USDT` đã pass full Prisma/Next/fault E2E và schema production rỗng đã áp an toàn. Còn identity QA/canary không tiền, migration số dư cũ, `Point -> YWH`, transfer/rút ngoài và contract hoa hồng YWH. Giữ debit tắt và callback public `404`; không chạy playbook tỷ giá cố định cũ.
...
- `[~]` **Nối web -> Point theo server-to-server - delivery đã chứng minh, synthetic đã được dọn:** ba lượt synthetic `+1 Point` từng đưa game `5000 -> 5003`; retry/realtime/relogin/restart/cross-device đều đúng. Sau các chi tiêu gameplay, preflight remediation thấy balance `4370` và đã đảo đúng `3 Point` về `4367` bằng ledger audit, không phải đặt lại `5003 -> 5000`. Identity canary `Nhien345` là tài khoản thật, nên không dùng tiếp làm QA. Nền candidate v3 đã deploy dormant nhưng còn identity QA/canary không tiền, kích hoạt, migration, YWH/USDT hai chiều/rút/hoa hồng và contract cuối; public callback tiếp tục `404`.
- `[x]` **Nghiệm thu ví Point duy nhất trong canary không tiền:** Windows EXE và Android APK đã tăng realtime, giữ số dư qua relogin/full process restart, đọc cùng `5003` xuyên thiết bị và thu hồi phiên cũ đúng hai chiều. Backend restart, fault/cron retry, client sau outage, isolated web restart + timeout sau commit, live off/on và relogin sau double restart đều đạt; ảnh HUD `5,003` là bằng chứng lịch sử trước các chi tiêu gameplay. Synthetic canary sau đó đã được reversal có audit; số dư authoritative hiện là `4367` và cần được client xác nhận ở lần đăng nhập kế tiếp.
