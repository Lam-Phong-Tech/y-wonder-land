...
>
> Trạng thái 11/07/2026: lõi **Giai đoạn 1 - Demo online nhanh** đã pass nhưng còn nghiệm thu tải 5–20 client và một số gameplay chưa server-authoritative. PostgreSQL production, backup/restore, private Node/Caddy staging, hardening API và full VPS reboot acceptance đã hoàn tất. Phần còn lại là xác nhận dependency DNS/web cũ, public HTTPS/WSS, đổi Unity URL và test tải.
...
- `[~]` Public cutover còn lại: `root/password` vẫn giữ để rollback; cần xác nhận quyền đổi DNS và dependency web cũ trên `api.ywonder.net`, đổi Caddy sang domain HTTPS/WSS, mở đúng `80/443` và test ngoài mạng. Sau khi đổi cấu hình Caddy phải lặp health/REST/WSS acceptance; tuyệt đối không public `3000/5432/8080`.
