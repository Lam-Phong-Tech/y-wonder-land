...
- `[x/~]` Report production `/root/ywonder-point-reports/point-wallet-migration-dry-run-20260716T134826Z-7dffc8b5.json`, SHA-256 `3ef6343bbef65bcfd35bce78aab10408df24c71c3c5fa90acd800788f3f14f16`, đã sinh an toàn nhưng gate migration **chưa đạt**: `BLOCKED=3`; 6 opening seed `5000 Point`, 7 legacy account và transaction history còn cần quyết định. Không deploy/migrate cho tới khi blocker về 0, có bảng phân loại từng HMAC account ref, QA identity riêng và duyệt rollback/release độc lập.
...
0. `[~]` **Ví Point chung:** production dry-run ẩn danh đã chạy và runner đạt, nhưng migration gate chưa đạt. Bước kế là lập quyết định đối soát cho 3 account có residual SWAP dưới 1 micro-Point, phân loại opening seed `5000 Point` của 6 game identity và 7 account legacy; không sửa balance trong bước phân loại. Sau đó mới khóa fail-closed mọi action web còn ghi `balanceGXL` đối với linked account và làm web-side reserve/capture/release outbox cho `Point -> USDT/YWH`. Chỉ khi `BLOCKED=0` và bảng quyết định được duyệt mới xin deploy candidate/migration riêng.
