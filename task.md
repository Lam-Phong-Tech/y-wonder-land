...
6. `[~]` **VÍ CHUNG - authority v3 + QA canary không tiền đã đạt:** Point web/game là một ví; game PostgreSQL là ledger. `USDT -> Point` dùng rate version Admin; canary QA đã chạy đúng UI web, journal/outbox/HMAC/game ledger và client EXE/APK với `5000 -> 5053`. Reserve/capture/release và web saga `Point -> USDT` đã pass cô lập nhưng production debit vẫn tắt. Còn canary tiền thật, migration số dư cũ, positive grant hardening, `Point -> YWH`, transfer/rút ngoài và contract hoa hồng YWH; callback public tiếp tục `404`.
