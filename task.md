...
- `[~]` **Nấc B Browser SSO - backend/web `[x]`, client artifact chờ test:** web build `hp9br9UtY4p9PcC214CmF` đã deploy callback/preserve login-register-OTP. Current game release `f75a7d6b3c5c267fbdf17f58af7d02bdecf8d5b9`, sau hotfix limiter `fc23f1652a8e484b42e348150d3a5a038825a2e0`: polling không còn ăn quota password (`125` poll public pass, start kế tiếp `201`), session web ghi nhớ tự approve qua callback, session chưa login tự đi qua `/vi/login`. Hai acceptance liên tiếp giữ cùng `PLAYER=p_1783873094`, profile `TRAN TUNG LAM`, Point `5000`; local/web credential vẫn song song. Unity đã bật feature flag và Android wake URI. Còn test Editor, EXE/APK, đăng ký account mới + OTP, Android quay lại app và relogin giữ cùng playerId/dữ liệu.

#### Cổng bàn giao tiếp theo
- `[~]` **MVP cho sếp/khách trải nghiệm - còn 3 cổng:** (1) runtime-test bốn nút account trong Editor, gồm web browser quay về game; (2) build EXE/APK đúng commit và smoke test local + web login + web register/OTP; (3) bàn giao artifact, tài liệu tester và kết quả test 2–5 thiết bị. Hạ tầng Nấc B đã pass, nhưng chưa gọi client artifact hoàn tất trước ba cổng này.
