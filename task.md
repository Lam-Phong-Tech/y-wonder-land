...
- `[x]` Đã audit tài nguyên/tiền/túi đồ/shop/farm-state tại `docs/PHASE1_STATE_SYNC_AUDIT.md`: profile có đọc/ghi server; economy/inventory mới chỉ load từ bootstrap; shop, farm và daily limits vẫn ghi local `PlayerPrefs`; PostgreSQL adapter còn là scaffold. Kết luận dự án ở cuối Giai đoạn 1 nhưng chưa đạt vòng dữ liệu server-authoritative.
