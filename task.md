...
>
> Worksheet quyết định root-only đã được sinh từ đúng report/checksum bằng generator fail-closed: 16 account ẩn danh, 21 quyết định còn `PENDING`, gồm 6 opening seed tổng `30000 Point`, synthetic canary `3 Point`, toàn bộ `3422.666667 Point` web legacy và 3 residual account vẫn `BLOCKED`. Một account đã map đồng thời có `12 Point` commission web và seed game, nên chưa thể tự chọn quy tắc cộng/trừ. Lượt này không gọi DB/service và không đổi production. Bước kế tiếp là lấy phê duyệt Product/Finance cho từng decision key, không phải viết migration.
>
...
- `[x/~]` Report production `/root/ywonder-point-reports/point-wallet-migration-dry-run-20260716T134826Z-7dffc8b5.json`, SHA-256 `3ef6343bbef65bcfd35bce78aab10408df24c71c3c5fa90acd800788f3f14f16`, đã sinh an toàn nhưng gate migration **chưa đạt**: `BLOCKED=3`. Worksheet JSON/Markdown mode `0600` đã tách 16 account thành 21 decision key, nhưng tất cả còn `PENDING`; 6 opening seed `5000 Point`, 7 legacy account, 3 residual account và transaction history chưa được Product/Finance chọn cách xử lý. Không deploy/migrate cho tới khi có phê duyệt từng HMAC account ref, chạy lại dry-run đạt `BLOCKED=0`, có QA identity riêng và duyệt rollback/release độc lập.
...
0. `[~]` **Ví Point chung:** production dry-run và worksheet ẩn danh đã đạt về tooling; không có DB mutation. Còn 21 quyết định `PENDING` cần Product/Finance duyệt: precision residual, nghĩa vụ rút của seed `5000 Point`, xử lý 3 Point synthetic và semantics balance/commission legacy. Sau khi điền quyết định, phải kiểm validator, chạy lại dry-run tới `BLOCKED=0`; rồi mới khóa fail-closed mọi action web còn ghi `balanceGXL` đối với linked account và làm web-side reserve/capture/release outbox cho `Point -> USDT/YWH`. Migration/deploy luôn là lượt riêng có backup/rollback.
