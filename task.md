...
## Ưu tiên hiện tại 16/07/2026: hoàn thiện nền ví Point chung trước, chưa deploy

> Phase 1 online cơ bản đã đạt. BA/khách đã xác nhận Point trên web và Point trong game là cùng một loại tiền, phải hiển thị cùng số dư như một ví; USDT đổi sang Point, YWH đổi hai chiều với Point, Point có thể đổi về USDT và tỷ giá do Admin thay đổi. Bản ghi nguồn chuẩn nằm tại `docs/POINT_WALLET_BUSINESS_RULES.md`; quyết định kỹ thuật nằm tại `docs/ADR_POINT_WALLET_AUTHORITY.md`. Candidate v3 đã chọn PostgreSQL game làm ledger Point duy nhất, đọc balance về web, dùng rate Admin có version và có state machine debit `reserve/capture/release`; vẫn chưa deploy/bật tiền thật.
>
> Production dry-run read-only 16/07/2026 đã chạy thành công bằng commit `7dffc8b5`: snapshot kép web/game không drift, raw/temp đã xóa, PID/build/env/health giữ nguyên và không có DB mutation. Report ẩn danh có 159 account: `NO_ACTION=143`, `UNMAPPED_LEGACY_REVIEW=7`, `MANUAL_RECONCILIATION_REQUIRED=6`, `BLOCKED=3`. Ba blocker là 6 giá trị SWAP/ví legacy có phần lẻ dưới 1 micro-Point; 6 game identity đều có opening seed đúng `5000` Point cần phân loại bằng văn bản. Ba outbox canary `1 Point` đều `SENT` và khớp đúng một game credit, tổng gửi/nhận cùng `3 Point`, không cộng đôi. Internal top-up vẫn `canary` một account thật, debit wallet tắt và callback public `404`; candidate v3 **chưa deploy** và chưa migrate balance.
>
> Chủ dự án đã duyệt policy cho đủ 21/21 decision item: opening seed và balance web legacy đều `DEFER_ACCOUNT_LINK`; synthetic canary chọn `AUDITED_REVERSAL_BEFORE_OPEN`; lịch sử balance 0 chọn archive không migrate; residual chọn `ROUND_HALF_EVEN` kèm audit. Applicator fail-closed đã áp policy lên đúng worksheet/checksum và sinh artifact root-only `0600`. Đây chỉ là duyệt quy tắc: `BLOCKED=3` vẫn còn, migration gate là `BLOCKED_NO_BALANCE_MIGRATION_AUTHORIZED`; chưa đảo 3 Point, chưa normalize residual, chưa link/migrate account, chưa deploy/restart/sửa DB.
>
> Remediation dry-run production đã đạt `[x]`: planner fail-closed đối chiếu lại snapshot kép hiện tại với đúng approved artifact/checksum, xác minh đủ ba source synthetic và sáu residual rồi chỉ sinh kế hoạch ẩn danh. JSON/Markdown root-only `point-wallet-remediation-dry-run-20260716T155640Z-4677eae4.*` có SHA-256 `fdd118a89f0ac70a60a96eb587d5f7a89b8dd731f02a9922d96410fda2b65357` / `975342dc9bec62f6166e7ef62d9fb657ff36d78a0f3028f902470320dd279416`. Kế hoạch dự kiến reversal đúng `3 Point` và sau residual normalization được duyệt thì blocker có thể từ `3 -> 0`, nhưng mọi operation vẫn `NOT_AUTHORIZED`: production hiện vẫn còn `5003 Point` ở account canary và `BLOCKED=3`; chưa có DB write, reversal, normalization, link, migration, deploy hoặc restart.
>
...
- `[x]` Người dùng nạp USDT rồi đổi sang Point; cho phép `YWH <-> Point` và `Point -> USDT` để rút; tỷ giá do Admin thay đổi, không dùng cố định `0,06 USDT/Point` hoặc `25 Point/USDT`.
- `[x]` Tiêu dùng trong game phải phát sinh hoa hồng cho người giới thiệu bằng YWH tương tự HUB. Phạm vi gồm mua vật nuôi, cây dài/ngắn ngày, mồi câu, lượt vòng quay, lượt đào khoáng và nói chung mọi tiêu dùng game.
- `[~]` BA trả lời hoa hồng/referral, quest, đầu tư/hoàn vốn/lãi, đổi quà/staking, `YWH -> Point` và chuyển Point không làm đổi Point game. Câu này mâu thuẫn với yêu cầu một ví và chuyển đổi hai chiều; phải chốt lại semantics trước khi viết contract.
...
- `[ ]` Còn phải làm web orchestrator/outbox cho `Point -> USDT/YWH`, reconciliation/migration từng account, Point transfer nếu được xác nhận và outbox hoa hồng YWH sau khi có công thức.
...
- `[x/~]` Report production `/root/ywonder-point-reports/point-wallet-migration-dry-run-20260716T134826Z-7dffc8b5.json`, SHA-256 `3ef6343bbef65bcfd35bce78aab10408df24c71c3c5fa90acd800788f3f14f16`, đã sinh an toàn nhưng gate migration **chưa đạt**: `BLOCKED=3`. Policy ở `docs/QA/POINT_WALLET_MIGRATION_APPROVED_POLICY_2026-07-16.json` đã được validator áp lên đúng worksheet: 16 account, 21/21 decision `APPROVED`, pending `0`. Artifact JSON/Markdown root-only cuối có SHA-256 `76c3368cc5937db5c93d8adbebb23d5abe2f2a8c279dd9f8d1daada5ddc7ca37` / `dd7a6190cdf7e5d8fe4c55a3b59275adb08921173e70eb1330ced3c863355282`. Remediation manifest production đã lập xong với SHA-256 `fdd118a89f0ac70a60a96eb587d5f7a89b8dd731f02a9922d96410fda2b65357` / `975342dc9bec62f6166e7ef62d9fb657ff36d78a0f3028f902470320dd279416`: đúng `3 Point`, 3 source, 3 account residual và 6 giá trị; projection sau change được duyệt là `BLOCKED=0`. Đây vẫn chỉ là plan `NOT_AUTHORIZED`; account link/migration tiếp tục defer và chưa có dữ liệu nào được sửa. Không deploy/migrate cho tới khi hai operation được duyệt, thực hiện nguyên tử có backup/audit, chạy lại dry-run đạt `BLOCKED=0`, có QA identity riêng và duyệt rollback/release độc lập.
...
0. `[~]` **Ví Point chung:** production dry-run, worksheet, policy approval và remediation manifest đều đạt mà không có DB mutation. Manifest đã khóa chính xác reversal synthetic `3 Point` cùng sáu residual `ROUND_HALF_EVEN`, nhưng tất cả operation là `NOT_AUTHORIZED`. Việc tiếp theo là xin duyệt vận hành riêng cho từng change, chuẩn bị backup/transaction bất biến/rollback, thực hiện nguyên tử rồi chạy lại dry-run tới `BLOCKED=0`; account link và balance migration vẫn defer. Song song mới hoàn thiện web-side reserve/capture/release outbox cho `Point -> USDT/YWH` trong môi trường cô lập; migration/deploy luôn là lượt riêng.
...
2. `[~]` **Retest hotfix gameplay đang chờ artifact:** hủy cho thú ăn phải dừng coroutine/animation và hoàn thức ăn; cây/thú phải chết hoặc héo ngay ở `0%`, persist qua relogin và EXE -> APK -> EXE.
...
4. `[~]` **Audit QARich và chi phí xây dựng - source đã đạt, chờ runtime/DB:** không có bypass theo account; `FarmScene` serialize đúng ruộng miễn phí, đường `4 đá`, chuồng `4 gỗ` và UI truyền đúng cost sang placement. Điểm trừ nay bắt buộc `RemoveItem` thành công, dùng reason `build_place` và log `before/after` để đối soát; Windows/Android compile sạch. Còn test QARich trước-sau-relogin và audit transaction. Đặt công trình + trừ vật liệu vẫn chưa phải một transaction server nguyên tử nên chưa được đánh dấu `[x]`.
5. `[ ]` **P0 mới - một cây có thể cộng gỗ hai lần sau khi đã biến mất, mới audit/chưa sửa:** runtime được báo cây đã chặt xong và biến mất nhưng prompt `Chặt cây` vẫn còn khi đứng sát; bấm lại có biểu hiện nhận thêm gỗ. Nguyên nhân client/prefab có bằng chứng trực tiếp: `Assets/_Project/Prefabs/Tree.prefab` đang chứa hai `HarvestableResource` cho cùng một cây, một ở root và một ở child `Log`; duplicate này đã có trong `HEAD`, không phải do chỉnh vị trí prefab đang dở. Audit YAML thấy `FarmScene` có 4 instance prefab cây và cả 4 đều giữ hai component; `CityScene` có 13 instance, trong đó 6 giữ cả hai và 7 đã remove component root nhưng vẫn giữ component child. `RealtimeClient` tạo network resource ID theo từng component/hierarchy nên backend coi đây là hai tài nguyên hợp lệ khác nhau và có thể thưởng một lần cho mỗi ID. Ngoài ra prompt direct-tap chỉ kiểm tra khoảng cách, không kiểm tra `isHarvestable`, còn `OnSharedResourceUnavailable` chưa xóa `currentHoverObject/currentActions` và HUD prompt. Backend PostgreSQL đã idempotent với cùng `resourceId + cycle + playerId`, nên chưa có bằng chứng DB tự cộng lặp cùng một giao dịch. Retry cùng ID của chính người thu hoạch có thể được server trả lại cached result với `duplicate=true`, nhưng model client hiện bỏ cờ này và vẫn hiện toast/cộng EXP như lượt mới; trường hợp đó không tăng inventory PostgreSQL nhưng gây cảm giác nhận gỗ lại. Khoảng trống backend còn lại là manifest tài nguyên vẫn do client khai báo nên chưa nhận biết hai ID cùng đại diện một cây vật lý. **Chưa sửa file gameplay/prefab.** Khi xử lý phải nghiệm thu: một cây vật lý chỉ có một resource authoritative; prompt tắt ngay khi depleted; bấm/retry trước respawn không animation, không toast thưởng/EXP và túi không tăng; respawn server chỉ mở đúng một cycle; unknown/trùng manifest bị từ chối hoặc chuẩn hóa server-side.
6. `[~]` **VÍ CHUNG - nền authority/rate/debit đã pass cô lập, chưa deploy:** Point web/game là một ví; game PostgreSQL đã được chọn làm ledger. USDT -> Point candidate dùng rate version Admin; reserve/capture/release game đã pass JSON/PostgreSQL race/restart. Còn web orchestrator chiều debit, migration số dư cũ, các semantics YWH/transfer và contract hoa hồng YWH. Giữ callback public `404`; không chạy playbook tỷ giá cố định cũ.
...
- `[~]` **Nối web -> Point theo server-to-server - bằng chứng kỹ thuật đạt, thiết kế nghiệp vụ cần làm lại:** ba lượt synthetic `+1 Point` đã đưa game `5000 -> 5003`; retry/realtime/relogin/restart/cross-device đều đúng và public callback vẫn `404`. Identity canary `Nhien345` nay được xác nhận là tài khoản thật, nên không dùng tiếp làm QA. Luồng cũ là delivery vào PostgreSQL game, chưa chứng minh một ví chung, tỷ giá động, YWH/USDT hai chiều, rút hoặc hoa hồng; không chạy giao dịch thật theo giả định cũ.
- `[x]` **Nghiệm thu ví Point duy nhất trong canary không tiền:** Windows EXE và Android APK đã tăng realtime, giữ số dư qua relogin/full process restart, đọc cùng `5003` xuyên thiết bị và thu hồi phiên cũ đúng hai chiều. Backend restart, fault/cron retry, client sau outage, isolated web restart + timeout sau commit, live off/on và relogin sau double restart đều đạt; ảnh cuối xác nhận HUD `5,003` khớp PostgreSQL.
