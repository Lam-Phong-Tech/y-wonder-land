...
- `[~]` Reconciliation read-only đã chốt `BLOCKED=0` và 17 decision còn hiệu lực; còn tạo successor policy checksum-pinned không cấp quyền ghi, làm web orchestrator/outbox cho `Point -> USDT/YWH`, migration/link từng account, Point transfer nếu được xác nhận và outbox hoa hồng YWH sau khi có công thức.
...
0. `[~]` **Ví Point chung:** remediation, client acceptance và fresh reconciliation đều đạt: residual `BLOCKED=0`, synthetic `4370 -> 4367`, EXE -> APK -> EXE giữ `4367`, report nhận đúng `3 Point` đã reversal và worksheet còn 17 decision không bao gồm synthetic/residual. Việc tiếp theo là ghim successor policy chỉ cho 17 decision defer/archive, rồi hoàn thiện web-side orchestrator/outbox `Point -> USDT/YWH`, chốt semantics transfer/YWH/hoa hồng và xin duyệt deploy candidate v3 riêng. Giữ `mode=canary` đúng một identity, public callback `404`, không link/migrate, không mở `open` và không thử tiền thật khi chưa có QA/phê duyệt.
