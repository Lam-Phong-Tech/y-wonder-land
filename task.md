...
- `[x]` **Nấc A web credential chạy song song, không thay luồng cũ:** release `5db92436a7974b38866fa3291f5f3e3577a2f30f` đã deploy versioned sau backup PostgreSQL/env/unit; production chạy `WEB_AUTH_MODE=http`, `AUTH_TRANSITION_MODE=parallel`, giữ đăng nhập/đăng ký local. Secret chỉ được copy nội bộ trong VPS, không in/lưu. Public acceptance bằng một account web thật và một account game local đã pass login -> bootstrap -> relogin, stable playerId và tách dữ liệu. Unity nay tách rõ `ĐĂNG NHẬP TRONG GAME` / `ĐĂNG NHẬP WEBSITE`; form đăng ký dùng `ĐĂNG KÝ TRONG GAME` / `ĐĂNG KÝ TRONG WEBSITE`. Đăng nhập website xác thực credential qua bridge, còn đăng ký website mở `https://ywonder.net/vi/login`. Editor compile sạch; chờ anh runtime-test hai lựa chọn rồi build. **Browser callback/PKCE SSO là Nấc B riêng, chưa sửa web/Nginx và chưa cutover khỏi local.**

#### Cổng bàn giao tiếp theo
- `[~]` **MVP cho sếp/khách trải nghiệm - còn 3 cổng:** (1) runtime-test bốn nút account trong Editor; (2) build EXE/APK đúng commit và smoke test local + web account; (3) bàn giao artifact, tài liệu tester và kết quả test 2–5 thiết bị. Nấc B không chặn MVP nếu sếp chấp nhận người chơi nhập credential web ngay trong game.
- `[ ]` **Production hoàn chỉnh - còn 4 nhóm lớn:** (1) Nấc B browser SSO callback/exchange + PKCE; (2) farm hai chiều xuyên thiết bị kèm revision/conflict; (3) server-authoritative cho gameplay còn lại và trạng thái resource qua restart; (4) monitoring/admin/audit/cert renewal cùng nghiệm thu 5–20 thiết bị thật. Không gộp bốn nhóm này vào cam kết thời gian bàn giao MVP.
