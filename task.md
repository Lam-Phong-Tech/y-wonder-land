...
- `[~]` Hạ tầng mới sếp cấp ngày 10/07: VPS ứng viên `42.96.18.14`, tài khoản quản trị `root`, mật khẩu giữ riêng ngoài repo. Lần kiểm tra đầu cổng SSH `22` từng phản hồi, nhưng lần kiểm tra lại từ máy làm việc ngày 10/07 không kết nối được `22/80/443/3000`; `api.ywonder.net` và `ywonder.net` hiện vẫn phân giải về `45.119.83.233`, chưa phải IP mới. Chưa đăng nhập/chưa thay đổi server/chưa chuyển build. Cần owner xác nhận đúng IP, trạng thái máy và firewall/whitelist; khi truy cập được mới audit read-only OS/tài nguyên, rồi tạo deploy user/SSH key, HTTPS/WebSocket, auto-start và backup. Thông tin này không làm đổi API hay chặn kế hoạch Phase 1 hiện tại.
