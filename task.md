...
- `[x/~]` Reconciliation read-only đã chốt `BLOCKED=0`; successor policy checksum-pinned đã duyệt đủ 17 decision còn hiệu lực nhưng không cấp quyền ghi/link/migrate. Web saga `Point -> USDT` đã pass full isolated validator với reserve/journal/capture/release, rate+fee snapshot, retry/idempotency và fault E2E; còn deploy candidate với debit tắt. `Point -> YWH`, migration/link từng account, Point transfer và outbox hoa hồng YWH vẫn chờ contract/phê duyệt.
...
0. `[~]` **Ví Point chung:** remediation, client acceptance, fresh reconciliation, successor policy và full isolated validator của web saga `Point -> USDT` đều đạt; migration vẫn không được cấp quyền. Việc tiếp theo là xin duyệt deploy candidate riêng với `WEB_POINT_WALLET_DEBIT_ENABLED` vẫn hiệu lực `false`, có backup/rollback và regression web cũ. `Point -> YWH`, transfer, rút USDT bên ngoài và hoa hồng vẫn chưa hoàn tất. Giữ `mode=canary`, public callback `404`, không link/migrate, không mở `open` và không thử tiền thật.
...
6. `[~]` **VÍ CHUNG - authority v3 và extension debit web đã pass cô lập:** Point web/game là một ví; game PostgreSQL đã được chọn làm ledger. `USDT -> Point` dùng rate version Admin; reserve/capture/release game đã pass JSON/PostgreSQL race/restart; web saga `Point -> USDT` đã pass full Prisma/Next/fault E2E bản sao. Còn deploy candidate tắt, migration số dư cũ, `Point -> YWH`, transfer/rút ngoài và contract hoa hồng YWH. Giữ callback public `404`; không chạy playbook tỷ giá cố định cũ.
