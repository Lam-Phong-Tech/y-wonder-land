...
- `[x/~]` Reconciliation read-only đã chốt `BLOCKED=0`; successor policy checksum-pinned đã duyệt đủ 17 decision còn hiệu lực nhưng không cấp quyền ghi/link/migrate. Còn làm web orchestrator/outbox cho `Point -> USDT/YWH`, migration/link từng account, Point transfer nếu được xác nhận và outbox hoa hồng YWH sau khi có công thức.
...
0. `[~]` **Ví Point chung:** remediation, client acceptance, fresh reconciliation và successor policy đều đạt: residual `BLOCKED=0`, synthetic `4370 -> 4367`, EXE -> APK -> EXE giữ `4367`, report nhận đúng `3 Point` đã reversal, 17 decision còn lại đã approved nhưng migration vẫn không được cấp quyền. Việc tiếp theo là audit phạm vi file rồi hoàn thiện web-side orchestrator/outbox `Point -> USDT/YWH`, chốt semantics transfer/YWH/hoa hồng và xin duyệt candidate v3/deploy riêng. Giữ `mode=canary` đúng một identity, public callback `404`, không link/migrate, không mở `open` và không thử tiền thật khi chưa có QA/phê duyệt.
