...
- `[~]` Hạ tầng VPS bước kế: chờ anh chấp thuận hardening có rollback (tạo user deploy + SSH key, test login rồi mới hạn chế root/password), sau đó cài Node.js LTS + PostgreSQL + Caddy. Không public port `3000/5432`; chỉ mở `80/443` khi proxy sẵn sàng. Ubuntu thực tế là 22.04.5, khuyến nghị giữ để không chậm deadline trừ khi công ty bắt buộc 24.04.
