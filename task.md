...
- `[~]` **Nấc B Browser SSO - backend/web `[x]`, client artifact chờ test:** web build `hp9br9UtY4p9PcC214CmF` đã deploy callback/preserve login-register-OTP với backup `/var/backups/ywonder-web/browser-sso-20260712T185239Z`. Game release `67ff2565517875fcc48ea515f1fedbbf98f24b8a` đã apply migration `002`, bật PKCE request một lần và giữ mode `parallel`; public acceptance account web thật pass callback -> exchange -> bootstrap PostgreSQL. Unity đã bật feature flag và Android wake URI. Còn test Editor, EXE/APK, đăng ký account mới + OTP, Android quay lại app và login lại giữ cùng playerId/dữ liệu.
