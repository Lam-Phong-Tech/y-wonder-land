...
- `[ ]` Không deploy candidate cho tới khi có QA identity riêng, báo cáo migration từng account và duyệt rollback/release độc lập.
...
0. `[~]` **Ví Point chung:** hoàn thiện web-side reserve/capture/release outbox cho chiều Point -> USDT/YWH; tạo migration dry-run ẩn danh cho 6 mapping và 9 ví legacy có Point; khóa fail-closed mọi action web còn ghi `balanceGXL` đối với linked account; sau đó mới xin duyệt deploy candidate và QA no-money.
