...
- `[~]` Hạ tầng mới sếp cấp ngày 10/07: VPS ứng viên `42.96.18.14`, tài khoản quản trị `root`, mật khẩu giữ riêng ngoài repo. Đã kiểm tra TCP và xác nhận cổng SSH `22` đang mở; chưa đăng nhập/chưa thay đổi server. Chưa chuyển build sang VPS này; cần audit read-only OS/tài nguyên trước, sau đó tạo deploy user/SSH key, firewall, HTTPS/WebSocket, auto-start và backup. Thông tin này không làm đổi API hay chặn kế hoạch Phase 1 hiện tại.
