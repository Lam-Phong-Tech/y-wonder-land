...
- `[~]` **Nấc B Browser SSO - backend/web `[x]`, client artifact chờ test:** Editor đã đăng nhập browser, tạo nhân vật và relogin đúng cùng tài khoản web. EXE cũ polling/đăng nhập đúng nhưng cần người dùng tự đưa cửa sổ game lên trước; đồng thời `intent=register` từng duyệt nhầm session web cũ. Commit `c002dfa0` thêm focus Windows sau exchange và callback web đã deploy hotfix: public probe xác nhận `intent=register` trả `302` sang `/vi/register`, giữ callback qua OTP bằng `registration_completed=1`, không còn tự duyệt nhân vật cũ. Local/web credential vẫn song song. Còn build lại EXE từ `c002dfa0`, test đăng ký account website mới + OTP bằng mã giới thiệu hợp lệ, APK quay lại app và relogin/cross-device giữ cùng playerId/dữ liệu.

#### Cổng bàn giao tiếp theo
- `[~]` **MVP cho sếp/khách trải nghiệm - còn 2 cổng:** (1) build EXE/APK từ ít nhất commit `c002dfa0` và smoke test local + web login + web register/OTP + quay lại game; (2) bàn giao artifact, tài liệu tester và kết quả test 2–5 thiết bị. Browser SSO Editor và hotfix callback đăng ký đã pass; chưa gọi client artifact hoàn tất trước hai cổng này.
