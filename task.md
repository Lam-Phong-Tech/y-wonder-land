...
## Ưu tiên hiện tại 16/07/2026: remediation ví Point đã đạt, chưa bật tiền thật/open
...
0. `[~]` **Ví Point chung:** hai remediation operation và nghiệm thu client sau reversal đã đạt production: residual `BLOCKED=0`, synthetic canary `4370 -> 4367`, EXE -> APK -> EXE đều giữ `4367`, thay phiên 4008/toast đạt hai chiều, replay không ghi đôi, backup/audit/postflight đầy đủ và không restart service. Việc tiếp theo không còn là chạy lại hai operation này mà là hoàn thiện web-side orchestrator/outbox cho `Point -> USDT/YWH`, chạy reconciliation mới cho account link/opening seed/legacy balance, chốt semantics transfer/YWH/hoa hồng và xin duyệt deploy candidate v3 riêng. Giữ `mode=canary` đúng một identity, public callback `404`, không mở `open` và không thử tiền thật khi chưa có QA/phê duyệt.
