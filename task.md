...
5. `[~]` **P0 - một cây có thể cộng gỗ hai lần: prefab + Farm đã sửa (21/07), chờ runtime:** nguyên nhân gốc là `Assets/_Project/Prefabs/Tree.prefab` chứa hai `HarvestableResource` cho cùng một cây (một ở root cấu hình đúng `axe_01`/`wood_01`, một ở child `Log` để rỗng); `RealtimeClient.RegisterSharedResource` sinh networkId theo hierarchy path khi `resourceId` rỗng nên hai component thành hai networkId, backend coi là hai tài nguyên và thưởng một lần cho mỗi ID.
   ...
   - **Còn lại / lưu ý:** (a) chưa nghiệm thu runtime — collider nằm trên `Log` (component giữ lại) nên chặt cây về lý thuyết vẫn chạy, cần build EXE/APK chặt thử để xác nhận một cây chỉ cộng gỗ một lần, prompt tắt khi depleted, retry trước respawn không cộng lại. (b) 14 cây City giờ có stale `m_RemovedComponents` trỏ tới root đã xóa — Unity bỏ qua an toàn, tự dọn khi save scene. (c) child `Log` còn gắn một `FarmInteractionController` (singleton) nghi lạc chỗ — CHƯA đụng, ngoài phạm vi bug nhân đôi, chờ chủ dự án xem lại.
   - **(d) cờ `duplicate` — ĐÃ FIX 21/07 (bé):** server `realtimeServer.js:467-473` khi cùng người chơi request trùng 1 tài nguyên đã hái sẽ **phát lại kết quả cũ** (`accepted:true`, rewards gốc) kèm `duplicate:true`. Client cũ **không đọc cờ này** → `HandleSharedResourceHarvestResult` cộng EXP + toast thưởng lần hai (túi đồ an toàn vì server gửi snapshot authoritative, client set-not-add; nhưng **EXP là client-side không idempotent → tap trùng/retry phồng EXP + toast ảo**). Fix: thêm `bool duplicate` vào `ResourceHarvestResult` + parse ở `ReceiveResourceHarvestResult`, và cho `HandleSharedResourceHarvestResult` khi `duplicate` chỉ đồng bộ lượt đào rồi `return`, KHÔNG AddEXP/toast. Compile chờ Unity; nghiệm thu runtime: tap trùng cây/đá không phồng EXP, không hiện toast thưởng lặp.
   - **(e) hai điểm gia cố còn lại — cân nhắc bỏ qua:** `prompt direct-tap không kiểm isHarvestable` và `OnSharedResourceUnavailable chưa xóa currentHoverObject/currentActions` đều **đã được che bởi cơ chế collider-disable** (`HarvestableResource.SetHarvestable(false)` tắt `resourceCollider.enabled` + ẩn visual → tài nguyên cạn không raycast trúng, hover tự dọn ở frame kế). Giá trị thấp mà đụng code hover nóng → bé KHÔNG sửa. Backend PostgreSQL vẫn idempotent theo `resourceId + cycle + playerId`.
