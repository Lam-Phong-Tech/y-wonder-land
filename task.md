...
>
> Trạng thái production kiểm tra read-only 16/07/2026: internal top-up vẫn ở `canary`, allowlist có đúng 1 account thật; debit wallet mới chưa bật và callback public vẫn `404`. Web có 159 ví, 10 ví giữ tổng `3422.666667` Point; game có 31 economy, 6 identity đã map và cả 6 đều lệch balance web. Một mapped account giữ 12 Point web từ commission; một mapped account có 3 outbox `SENT` khớp đúng 3 ledger `web_topup_credit` phía game. Không được cộng toàn bộ balance web vào game. Không dùng tiền thật và candidate v3 **chưa deploy**.
>
...
- `[x]` Candidate migration dry-run đã pass local: SQLite `mode=ro/query_only`, PostgreSQL `REPEATABLE READ READ ONLY`, capture hai lượt chống drift, integer micros, HMAC ref và fail-closed duplicate/orphan/mismatch. Report không sinh SQL hoặc số tiền auto-migrate; runbook ở `docs/QA/POINT_WALLET_MIGRATION_DRY_RUN.md`.
- `[ ]` Chưa chạy dry-run trên production và chưa phân loại từng account. Không deploy candidate cho tới khi có report production read-only được duyệt, QA identity riêng và duyệt rollback/release độc lập.
...
0. `[~]` **Ví Point chung:** tooling migration dry-run ẩn danh đã pass local nhưng chưa chạy production. Bước kế là xin duyệt lượt production read-only, phân loại 6 mapping và 9 ví legacy có Point; sau đó khóa fail-closed mọi action web còn ghi `balanceGXL` đối với linked account, rồi làm web-side reserve/capture/release outbox cho `Point -> USDT/YWH`. Chỉ sau các cổng đó mới xin duyệt deploy candidate và QA no-money.
